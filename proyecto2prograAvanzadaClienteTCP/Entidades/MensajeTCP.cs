...
namespace Entidades
{
    public class MensajeTCP
    {
        ...
        public MensajeTCP(string comando, params string[] parametros)
        {
            Comando = comando;
        ...
        public static MensajeTCP Parse(string mensaje)
        {
            var partes = mensaje.Split('|');
            if (partes.Length == 0) return null;

            return new MensajeTCP
            {
                Comando = partes[0],
                Parametros = partes.Skip(1).ToList()
            };
        }
    }
}
